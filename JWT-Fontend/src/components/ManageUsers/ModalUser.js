...
const ModalUser = (props) => {
	...
	const [validInputs, setValidInputs] = useState(validInputesDefault);
	...
	}, [dataModalUser]); //mỗi lần biến này thay đổi thì nó sẽ chạy vào hàm useEffect này

	/* trog TH lần đầu tiên nhấn vào create, biến này chạy, mà chưa có data của userGroups thì sẽ ko chạy */
	useEffect(() => {
		if (action === 'CREATE') {
	...
	}, [action]); //VD: từ create sang update thì sẽ chạy vào hàm này
	...
	const checkValidateInputs = () => {
		...
		setValidInputs(validInputesDefault); //mỗi 1 lần tạo mới user thì sẽ phải set lại
		let arr = ['email', 'phone', 'password', 'group'];
		let check = true;
		for (let i = 0; i < arr.length; i++) {
			//input k dc để trống, nếu trống thì hiện thông báo
			if (!userData[arr[i]]) {
				let _validInputs = _.cloneDeep(validInputesDefault);
	...
	const handleCloseModalUser = () => {
		...
		setValidInputs(validInputesDefault);
	};
